...
#include <nvmtools/NVMCameraModel.h>
...
void Camera::init(const nvmtools::NVM_Camera* cam, int width, int height, const int maxLevel){
	CHECK(cam != nullptr && cam->intrinsics) << "incomplete input";

	// copy the intrinsics
	cameraModel_ = cam->intrinsics;
	if (cameraModel_->type() == nvmtools::IntrinsicsType::NVM_DEFAULT){
		cameraModel_->set(nvmtools::NVMCameraModel::PRINCIPAL_POINT_X, width / 2.0);
